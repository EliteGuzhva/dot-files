...
bgcolor='#01142F'  # $(hc get frame_border_normal_color)
...

{
    ...
    while true ; do
		...
		# Volume
		right="$right^bg($selbg)^fg(#ffffff)  Volume: $vol%  "
		symbols="$symbols$vol_icon"

		# Input method
		right="$right^bg(#cc3c3c)^fg(#ffffff) $input "

		# Date
		right="$right^bg(#3ccc3c)  $date  "

		# get text from the right part
        right_text_only=$(echo -n "$right" | sed 's.\^[^(]*([^)]*)..g')

        # get width of right aligned text.. and add some space..
		width_of_text=$($textwidth "$font" "$right_text_only") 
		width_of_icons=$($textwidth "$iconfont" "$symbols")
		width=$(( $width_of_text ))
